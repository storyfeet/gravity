...
use piston_window::{draw_state, image, line, Context, G2d, Transformed};
...
use super::rects::{shrink_by, Position, QTrans, DOWN, LEFT, RIGHT, UP};
use super::state::{AnimState, DrawCp, DrawMode, State, Tile};
...
pub fn tile_draw_sys(s: &State, tex_map: &TexLoader, c: Context, g: &mut G2d) {
    let mut ls_draw: Vec<DrawData> = s
        .tiles
        .iter()
    ...
    for ddat in ls_draw {
        ...
        let (npx, flip, txf) = match ddat.t {
            ...
            Tile::Block => (50., false, "assets/block.png"),
        ...
        let c = c
            ...
            .dir_about(25., 25., dir)
            .scale(50. / npx, 50. / npx);
        let c = if flip { c.trans(50., 0.).flip_h() } else { c };
        image(tx, c, g);
    }
}
...
pub fn grid_draw_sys(s: &State, tex_map: &TexLoader, c: Context, g: &mut G2d) {
    ...
    Rectangle::new([1., 1., 1., 1.]).border(border).draw(
        [0., 0., s.walls.w as f64 * 50., s.walls.h as f64 * 50.],
    ...
    for (i, w) in s.walls.v.iter().enumerate() {
        ...
        for dir in 0..4 {
            ...
            match w[dir] {
                ...
                Edge::Spike => {
                    if let Some((_, tx)) = tex_map.get_by_path("assets/spike.png") {
                        image(tx, c, g);
                    }
                }
                Edge::Door => {
                    if let Some((_, tx)) = tex_map.get_by_path("assets/door.png") {
                        image(tx, c, g);
                    }
                }
                _ => {}
    ...
    for (i, core) in s.walls.vc.iter().enumerate() {
        ...
        if let TileCore::GravChanger(dir) = core {
            if let Some((_, tx)) = tex_map.get_by_path("assets/arrow.png") {
                let c = c
                    .transform
                    .trans(x1, y1)
                    .dir_about(25., 25., *dir)
